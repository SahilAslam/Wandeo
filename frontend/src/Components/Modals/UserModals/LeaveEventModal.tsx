...
const LeaveEventModal: React.FC<LeaveEventProps> = ({ visible, closeModal, LeaveEvent }) => {
    const handleClick = () => {
        LeaveEvent();
        closeModal();
    }
          ...
          <div className="rounded-lg bg-white h-fit w-full sm:max-w-sm">
            <div className='flex justify-between items-center py-5 px-5 rounded-t-lg bg-slate-100'>
                <div className='flex items-center gap-1'>
                  ...
                  <h1 className='text-slate-700 text-xl font-semibold'>Leave Event</h1>
                </div>
                <IoMdClose className="text-slate-700 text-2xl" onClick={closeModal} />
            </div>
            <div className='px-5 py-10'>
                <p className='text-center'>Are you sure you want to Leave this Event?</p>
            </div>
            <div className='flex gap-2 justify-end px-5 py-5 bg-slate-100 rounded-b-lg'>
                ...
                <button onClick={handleClick} className='bg-red-600 rounded px-2 py-2 text-white'>Leave Event</button>
            </div>
          </div>
        </div>
      )}
